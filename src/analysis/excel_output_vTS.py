...
def output_parameters(net):
    ...
    parameters = {'net_load' : ['zone','bus','vn_kv','in_service'],
                  ...
                  'res_trafo' : ['p_from_mw', 'q_from_mvar', 'p_to_mw', 'q_to_mvar', 'pl_mw', 'ql_mvar', 'loading_percent'],
...
def create_excel(network_name, scenario_name, net, gen_fuel_tech, output_path , number, column, parameters):    
    # 
        ...
        if number['loads'] > 0:
            ...
            for i in range(len(parameters['net_load'])):
                if not parameters['net_load'][i] in net.load.keys():  #check the colum of parameter
                    net.load[parameters['net_load'][i]] = [None]*number['res_load'] #if the columm of parameter is empty, it creat that columm and put value as none
            ...
            for i in range(number['loads']): # extract the values from power flow results
                ...
                for j in range(len(load_row)): # going to all the values in the same line/row
                    load_line = i + initial_line + step*number['loads'] #values start in the row 4 of the sheet
                    load_cell = column['load'][j] + str(load_line) #update cell reference, from left to right 
        ...
        if number['generators'] > 0:  
            ...
            for i in range(number['generators']): # extract the values from power flow results
                ...
                for j in range(len(gen_row)): # going to all the values in the same line/row
                    gen_line = i + initial_line + step*number['generators'] #values start in the row 4 of the sheet
                    gen_cell = column['gen'][j] + str(gen_line) #update cell reference, from left to right 
        ...
        if number['lines'] > 0:
            ...
            for i in range(number['lines']): # extract the values from power flow results
                ...
                for j in range(len(line_row)): # going to all the values in the same line/row
                    line_line = i + initial_line + step*number['lines']#values start in the row 4 of the sheet
                    line_cell = column['line'][j] + str(line_line) #update cell reference, from left to right 
        ...
        if number['trafos'] > 0:
            ...
            for i in range(number['trafos']): # extract the values from power flow results
                ...
                for j in range(len(trafo_row)): # going to all the values in the same line/row
                    trafo_line = i + initial_line + step*number['trafos']#values start in the row 4 of the sheet
                    trafo_cell = column['trafo'][j] + str(trafo_line) #update cell reference, from left to right 
        ...
        if number['buses'] > 0:
            ...
            for i in range(number['buses']):
                ...
                for j in range(len(bus_row)):
                    bus_line = i + initial_line + step*number['buses']
                    bus_cell = column['bus'][j] + str(bus_line)
